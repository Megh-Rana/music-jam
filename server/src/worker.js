...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/extract' && request.method === 'POST') {
      if (env.ENABLE_YTDLP !== 'true') {
        return json({ error: 'Extraction mode is disabled on backend' }, 400)
      }
      const videoId = parseVideoId(String((await request.json().catch(() => ({}))).videoId || ''))
      if (!videoId) return json({ error: 'Invalid videoId' }, 400)
      const base = env.YTDLP_API_BASE || ''
      if (!base) return json({ error: 'Missing YTDLP_API_BASE' }, 400)

      const upstream = new URL('/extract', base)
      upstream.searchParams.set('videoId', videoId)
      const res = await fetch(upstream, {
        headers: env.YTDLP_API_KEY ? { 'x-api-key': env.YTDLP_API_KEY } : {},
      })
      if (!res.ok) return json({ error: 'Extractor service failed' }, 502)
      const data = await res.json()
      return json({
        videoId,
