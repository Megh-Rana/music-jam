...
On branch `yt-dlp`, set:

- `ENABLE_YTDLP=true`
- `YTDLP_API_BASE=https://your-extractor-service`
- `YTDLP_API_KEY=...` (optional)

`yt-dlp` cannot run inside Cloudflare Workers directly, so extraction mode uses an external extractor API.
